...
pub fn exec(
    ...
    for p in &targets {
        ...
        let kind = match p { Package::Coreutils => Some(PackageKind::Coreutils), Package::Findutils => Some(PackageKind::Findutils), Package::Sudo => None };
        if let Some(k) = kind {
            let source_bin = resolve_source_bin(*p);
            if let Err(missing) = coverage_preflight(&adapter, root, k, &source_bin) {
                return Err(format!("coverage preflight failed for {:?}: missing: {}", p, missing.join(", ")));
            }
        }
    }
    ...
    for p in &targets {
        if !matches!(p, Package::Sudo) {
            let kind = if matches!(p, Package::Coreutils) { PackageKind::Coreutils } else { PackageKind::Findutils };
    ...
    let (distro_names, _rs_names): (Vec<&str>, Vec<&str>) = targets.iter().map(|p| match p {
    ...
    if matches!(mode, ApplyMode::Commit) {
        if !live_root {
            eprintln!("[info] skipping pacman removals under non-live root: {}", root.display());
        } else {
            for name in distro_names {
                let mut cmd = Command::new("pacman");
                let args = vec!["-R".to_string(), "--noconfirm".to_string(), name.to_string()];
                ...
                if code != 0 { return Err(format!("pacman -R {} failed with exit code {}", name, code)); }
            }
        }
    } else {
        for name in distro_names { eprintln!("[dry-run] would run: pacman -R --noconfirm {}", name); }
    }
    ...
    for (pkg, names, src) in verify_sets {
        ...
        let src_sp = SafePath::from_rooted(root, &src).map_err(|e| format!("invalid source_bin: {e:?}"))?;
        ...
        for app in names {
            ...
            if !ok { return Err(format!("post-verify failed for {:?}: {} does not point to replacement", pkg, dst.display())); }
        }
    }
...
fn verify_link_points_to(dst: &Path, src: &Path) -> bool {
    ...
    if let Ok(md) = fs::symlink_metadata(dst) {
        if md.file_type().is_symlink() {
            if let Ok(cur) = fs::read_link(dst) { return cur == src; }
        }
    }
    false
}
...
fn resolve_source_bin(pkg: Package) -> PathBuf {
    ...
    match pkg { Package::Sudo => PathBuf::from("/usr/bin/sudo"), _ => PathBuf::from("/usr/bin/uutils") }
}
